...
using ModTogether.Plugins.MHW.Models;

namespace ModTogether.Plugins.MHW.Services
{
    public class ModInstaller
    {
        ...
        public void InstallMod(string archivePath, string relativeKey, List<string>? selectedFiles = null)
        {
            try
            {
                ...
                var allExtractedFiles = Directory.GetFiles(tempDir, "*.*", SearchOption.AllDirectories);
                int totalFiles = allExtractedFiles.Length;
                ...
                foreach (var file in allExtractedFiles)
                {
                    ...
                    if (totalFiles > 0)
                    {
                        double copyPct = 50.0 + ((double)processedFiles / totalFiles) * 50.0;
                        OnInstallProgress?.Invoke(copyPct);
                    }

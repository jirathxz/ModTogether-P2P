...
namespace ModTogether.Plugins.MHW
{
    ...
    public partial class ManagerPage : Page
    {
        ...
        private string? ResolveModPath(string filename)
        {
            string docsPath = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments);
            string allModsDir = System.IO.Path.Combine(docsPath, "ModTogether", "AllMods");
            string p1 = System.IO.Path.Combine(allModsDir, filename);
            if (System.IO.File.Exists(p1)) return p1;
            
            string mhwDir = App.Settings.Current.MhwDirectory;
            if (!string.IsNullOrEmpty(mhwDir))
            {
                string p2 = System.IO.Path.Combine(mhwDir, "GameMods", filename);
        ...
        private async void ManagerPage_Drop(object sender, DragEventArgs e)
        {
            ...
            if (e.Data.GetDataPresent(DataFormats.FileDrop))
            {
                ...
                if (_lblModInfo != null) _lblModInfo.Text = $"Importing {files.Length} file(s)... Please wait.";
                ...
                if (_lblModInfo != null) _lblModInfo.Text = "Import complete.";
        ...
        private async void BtnImport_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (dialog.ShowDialog() == true)
            {
                ...
                var loadingRing = this.FindName("LoadingRing") as Wpf.Ui.Controls.ProgressRing;
                if (loadingRing != null) loadingRing.Visibility = Visibility.Visible;
                if (_lblModInfo != null) _lblModInfo.Text = $"Importing {dialog.FileNames.Length} file(s)... Please wait.";
        ...
        private void ScanMods()
        {
            ...
            string docsPath = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments);
            string allModsDir = System.IO.Path.Combine(docsPath, "ModTogether", "AllMods");
            System.IO.Directory.CreateDirectory(allModsDir);
            
            var allFiles = new System.Collections.Generic.HashSet<string>(StringComparer.OrdinalIgnoreCase);
            
            try {
                foreach (var f in System.IO.Directory.GetFiles(allModsDir)) allFiles.Add(f);
            } catch {}
            try {
                foreach (var f in System.IO.Directory.GetFiles(cacheDir)) allFiles.Add(f);
            } catch {}
            ...
            foreach (var file in allFiles)
            {
                try
                {
                    ...
                    if (ext == ".zip" || ext == ".7z" || ext == ".rar")
                    {
                        ...
                        if (!processedNames.Add(filename)) continue;
        ...
        private async void ListMods_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            if (_listMods.SelectedItem is ModItemData item)
            {
                ...
                string? fullPath = ResolveModPath(filename);
                if (fullPath != null)
                {
        ...
        private async void BtnDelete_Click(object sender, RoutedEventArgs e)
        {
            if (_listMods.SelectedItem is ModItemData item)
            {
                ...
                await System.Threading.Tasks.Task.Run(async () => 
                {
                    ...
                    if (fullPath != null)
                    {
                        string recycleDir = System.IO.Path.Combine(System.IO.Path.GetDirectoryName(fullPath)!, ".recycle_mods");
        ...
        private async void BtnInstallChecked_Click(object sender, RoutedEventArgs e)
        {
            ...
            await System.Threading.Tasks.Task.Run(() =>
            {
                ...
                {
                    string? fullPath = ResolveModPath(item.Filename);
                    if (fullPath != null) App.Installer.InstallMod(fullPath, item.Filename);
                }
            });
